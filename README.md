...
```javascript
// 1 estrella
// Forma simple e inificiente de hacerlo:

function detectBombs(grid) {
  let response = grid.map(x => x.map(y =>0))
...
function detectBombs(grid) {
    ...
    const response = grid.map(x => x.map(_=>0))
...
```js
// 5 estrellas a la primera LOL
function findInAgenda(agenda, phone) {
  const regexName = /(?<=<)[a-zA-Z]+ [a-zA-Z]+(?=>)/g
  const regexNumber = /\+\d+-\d+-\d+-\d+/g
  const regexAddress = /(?<!<)(?<= )[\w\d]+(?: [\w\d]+)*(?= )(?!>)/g

  const names = [...agenda.matchAll(regexName)]
  const numbers = [...agenda.matchAll(regexNumber)]
  const addresses = [...agenda.matchAll(regexAddress)]

  let matches = {}
  let moreThanOne = 0
  for(let i in names){
    if(numbers[i][0].includes(phone)){
      matches['name']= names[i][0]
      matches['address']= addresses[i][0]
      moreThanOne +=1 
    }
  }


  return moreThanOne === 1?matches:null || null
}
```